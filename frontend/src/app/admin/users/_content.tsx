    ...
    return (
        <ProtectedRoute allowedRoles={['ADMIN', 'MANAGER']}>
            <div className="flex h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden">
                ...
                {/* Main Content */}
                <main className="flex-1 ml-60 p-8 overflow-y-auto bg-slate-50 dark:bg-slate-950">
                    {/* Header */}
                    <div className="mb-8">
                        <h1 className="text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">Organization</h1>
                        <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your team members and group them into projects.</p>
                    </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        {projects.map(project => (
                                            <div key={project.id} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-md transition-shadow">
                                                ...
                                                <div className="p-6">
                                                    ...
                                                    {(() => {
                                                        const availableUsers = users.filter(u => u.isActive && !project.projectMembers?.find(m => m.user.id === u.id));
                                                        return (
                                                            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
                                                                <div className="relative flex-1">
                                                                    <select
                                                                        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg pl-3 pr-9 py-2 text-sm text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-indigo-500 outline-none appearance-none disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
