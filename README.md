...
4. Push the Prisma schema to the database:
   ...
   npx prisma db push
   ```
5. Seed the database with initial roles, test users, and sample reports:
   ```bash
   npx prisma db seed
   ```
...
2. Install dependencies:
   ...
   npm install
   ```
3. Create a `.env.local` file in the `frontend` directory and define the backend API URL:
   ```env
